  ...
  if(pecas[indexEscolhido].team == 1){
    if(j1.x >= 0 && j1.x < qntCasas && j1.y >= 0 && j1.y < qntCasas){
      if(getIndexByPosition(pecas, j1.x, j1.y)==-1){
        circle(j1.x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, j1.y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);  
      }else if(pecas[getIndexByPosition(pecas, j1.x, j1.y)].team != pecas[indexEscolhido].team){
        if(getIndexByPosition(pecas, j1.x-1, j1.y-1)==-1){
          jogadas[0].x -=1;
          jogadas[0].y -=1;
          jogadas[0].xVitima = j1.x;
          jogadas[0].yVitima = j1.y;
          circle(jogadas[0].x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, jogadas[0].y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);
        }
      }
    }
    if(j4.x >= 0 && j4.x < qntCasas && j4.y >= 0 && j4.y < qntCasas){
      if(getIndexByPosition(pecas, j4.x, j4.y)==-1){
        circle(j4.x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, j4.y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);  
      }else if(pecas[getIndexByPosition(pecas, j4.x, j4.y)].team != pecas[indexEscolhido].team){
        ...
        if(getIndexByPosition(pecas, j4.x+1, j4.y-1)==-1){
          ...
          circle(jogadas[3].x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, jogadas[3].y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);
        }
      }
    }
  }else{
    if(j2.x >= 0 && j2.x < qntCasas && j2.y >= 0 && j2.y < qntCasas){
      if(getIndexByPosition(pecas, j2.x, j2.y)==-1){
        circle(j2.x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, j2.y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);  
      }else if(pecas[getIndexByPosition(pecas, j2.x, j2.y)].team != pecas[indexEscolhido].team){
        if(getIndexByPosition(pecas, j2.x-1, j2.y+1)==-1){
          jogadas[1].x -=1;
          jogadas[1].y +=1;
          jogadas[1].xVitima = j2.x;
          jogadas[1].yVitima = j2.y;
          circle(jogadas[1].x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, jogadas[1].y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);
        }
      }
    }
    if(j3.x >= 0 && j3.x < qntCasas && j3.y >= 0 && j3.y < qntCasas){
      if(getIndexByPosition(pecas, j3.x, j3.y)==-1){
        circle(j3.x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, j3.y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);  
      }else if(pecas[getIndexByPosition(pecas, j3.x, j3.y)].team != pecas[indexEscolhido].team){
        if(getIndexByPosition(pecas, j3.x+1, j3.y+1)==-1){
          jogadas[2].x +=1;
          jogadas[2].y +=1;
          jogadas[2].xVitima = j3.x;
          jogadas[2].yVitima = j3.y;
          circle(jogadas[2].x * ((float)largura / qntCasas)+((float)largura / qntCasas)/2, jogadas[2].y * ((float)altura / qntCasas)+ ((float)largura / qntCasas)/2, 60);
        }
      }
    }
  }
...
void gerarMarcacoes(){
  ...
  for(int i = 0; i < qntCasas; i++){
    text(tamanhoCelula*i+20, 615, "x-%d", i);
  }
  for(int i = 0; i < qntCasas; i++){
    text(607, tamanhoCelula*i+20, "y-%d", i);
  }
}
...
int main() {
  ...
  while(1){
    ...
    if(pecas[pecaEscolhida].team == vez-1){
      ...
      for(int i = 0; i<4; i++){
        if(x == jogadas[i].x && y == jogadas[i].y && getIndexByPosition(pecas, x, y) == -1){
          pecas[pecaEscolhida].x = x;
          pecas[pecaEscolhida].y = y;
          if(jogadas[i].xVitima != -1){
            deletePeca(pecas, getIndexByPosition(pecas, jogadas[i].xVitima, jogadas[i].yVitima));
          }
          break;
        }
      }
      gerarMapa(pecas);
      save();
      if(vez == 1 && (pecas[pecaEscolhida].x != beforeX || pecas[pecaEscolhida].y != beforeY)){
      vez = 2;
      }else{
        vez = 1;
      }
